...
classdef SOMFNN_GPU
    ...
    methods
        %% ----------------------- construct ------------------------------
        function o = SOMFNN_GPU(Xtr, Ytr, n_Layer, opts)
            arguments
                ...
                opts.dropRuleRegularization = 0
                opts.dropRuleRate = 50
                opts.useGpu = false
            end

            o.useGpu = opts.useGpu;
            ...
            o.dropRuleRegularization = opts.dropRuleRegularization;
            o.dropRuleRate = opts.dropRuleRate;
            o.useGpu = opts.useGpu;
        ...
        function [trained_net,valData] = Train(o,opts)
                ...
                if o.Verbose
                    if ~rem(epoch,o.Verbose)
                        if o.regression_flag
                            ...
                            fprintf("[Epoch:%d] [ACC:%.3f] [Loss:%.4f] [TimeExe:%.2f] \n", epoch, METRIC_tr(epoch), LOSS_tr(epoch), toc)
                        end
                    end
                end
            ...
            if opts.ApplyRuleRemover
                if o.validation_flag && epoch>1
                    o = o.RulePruning(Xval,0.5,0.5);
                    netBest = netBest.RuleRemover(Xval,0.5,0.5);
                else
                    o = o.RulePruning(o.Xtrain,0.5,0.5);
                end
            end
        ...
        %% ------------------------ Rule Pruning ---------------------------
        function outNets = RulePruning(net,InputData,percent,k,opts)
            arguments
                net
                InputData
                percent
        ...
        function net = RulePruning2(net,X,Y)
            [~,metric,~] = net.Test(X,Y);
            metric = metric.MSEorACC;
            for L = 1:net.n_Layer
                R = 0;
                while true
                    R = R + 1;
                    pruned_net = prune(net, L, R);
                    [~,pruned_metric,~] = pruned_net.Test(X,Y);
                    pruned_metric = pruned_metric.MSEorACC;
                    if checkNets
                        net = pruned_net;
                        metric = pruned_metric;
                        R = R - 1;
                    end
                    if R == net.Layer{L}.N, break, end
                end
            end

            function net = prune(net,l,n)
                % prune one rule
                net.Layer{l}.N = net.Layer{l}.N - 1;
                net.Layer{l}.CS(n) = [];
                net.Layer{l}.Cc(:,n) = [];
                net.Layer{l}.CX(n) = [];
                net.Layer{l}.P(:,n) = [];
                w = net.Layer{l}.W;
                net.Layer{l}.A(w*(n-1)+1:w*n,:) = [];
                net.n_rulePerLayer(l) = net.Layer{l}.N;
            end
            function flag = checkNets
                flag = 0;
                if net.classification_flag
                    if pruned_metric > metric, flag = 1; end
                else
                    if pruned_metric < metric, flag = 1; end
                end
            end
    ...
    methods (Access=private)
        ...
        function [y,o,BP_pars] = Forward(o,x)
            ...
            if o.dropRule_flag
                prob = rand(1,sum(o.n_rulePerLayer));
                prob = prob >= prctile(prob(:), o.dropRuleRate*100); % remain=1, drops=0
                for l = 1:o.n_Layer
                    o.prob_mat{l} = prob(sum(o.n_rulePerLayer(1:l-1))+1:sum(o.n_rulePerLayer(1:l)));
                    if ~sum(o.prob_mat{l})
                        o.prob_mat{l}(randi(numel(o.prob_mat{l}))) = 1;
                    end
                end

                % o.prob_mat = rand(max(o.n_rulePerLayer),o.n_Layer);
                % for l = 1:o.n_Layer, o.prob_mat(o.Layer{l}.N+1:end,l) = inf; end
                % o.prob_mat = o.prob_mat >= prctile(o.prob_mat(:), o.dropRuleRate*100); % remain=1, drops=0
                % sum(o.prob_mat)
            end
            ...
            for l = 1:numel(o.Layer)
                ...
                MB = size(x,2);
                for k = 1:MB
                    [outLay,o] = o.RuleFunc(o.Layer{l}, x(:,k), o.dataSeenCounter+k);
                    o.Layer{l} = outLay;
                ...
                N = o.Layer{l}.N;
                N_old = o.n_rulePerLayer(l);
                o.n_rulePerLayer(l) = N;
                n = 1:N;
                M = o.Layer{l}.M;
                W = o.Layer{l}.W;

                [lambda_l,~,~] = o.GetLambda(x,o.Layer{l},n);
                if o.dropRule_flag
                    lambda_l(~o.prob_mat{l},:) = 0;
                end
                % lambda_l=o.setDev(lambda_l);
                x=o.setDev(x);
        ...
        %% ------------------------------------- ADD OR UPDATE RULE  -------
        % used in @forward
        function [l,o] = RuleFunc(o,l,xk,dataSeen)
            ...
            if l.N == 0 % Stage(0)
                ...
                if max_dens < o.DensityThreshold
                    ...
                    if o.dropRule_flag
                        o.prob_mat{l.NO} = [o.prob_mat{l.NO},1];
                    end
                else
                    if o.dropRule_flag
                        if o.prob_mat{l.NO}(n_star)
                            %%%%% UPDATE %%%%%
        ...
        function o = Backward(o,yh,y,BP_pars,it)
            ...
            for  l = o.n_Layer : -1 : 1
                ...
                P_4D = o.setDev(reshape(o.Layer{l}.P(:,n), M,1,N)); %(M,N)=>(M,1,N,MB=NoNeed)
                taw2_4D = o.setDev(reshape(o.GetTau2(o.Layer{l},n), 1,1,N)); %(N,1)=>(1,1,N,MB=NoNeed)
                ...
                if o.dropRule_flag
                    rem_rules = logical(repelem(o.prob_mat{l}',W,1));
                else
                    rem_rules = true(N*W,1);
                end
                o.Layer{l}.A = (o.Layer{l}.A);
