function [Output]=SAFLS(Input,Mode)
    ...
    for ii=2:1:L
        ...
        if max(LocalDensity)<threshold1
            %% new_cloud_add
            ModelNumber=ModelNumber+1; % N
            center=[center;datain];  % Cc
            prototypes=[prototypes;datain]; % P
            local_X=[local_X;datain.^2]; % CX @
            Support=[Support,1]; % CS
            local_delta=[local_delta;zeros(1,W)];
            A(:,:,ModelNumber)=mean(A,3); % A   @
